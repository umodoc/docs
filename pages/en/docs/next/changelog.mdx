...
1. Added the new **Business Blocks** feature. See: [Business Blocks](./biz-block);
2. Fixed an issue where inserting multiple selected images could cause them to overwrite each other;
3. Adjusted the zoom threshold for image preview;
4. Improved the display of images and tables after Word export;
5. Refreshing in read-only mode no longer prompts for unsaved changes;
6. Fixed an issue where excessive toolbar scrolling could prevent some menu items from ever becoming visible;
7. Optimized insertion and export behavior for SVG diagram nodes such as flowchart, Mermaid, and PlantUML;
8. Improved clarity when exporting image nodes to Word documents;
9. Fixed an issue where child lists did not update their numbering type after switching list numbering in the menu;
10. Improved insertion behavior for Mermaid and other SVG image nodes;
11. Fixed an issue where excessive toolbar scrolling could prevent some menu items from ever becoming visible;
12. Other minor refinements.

## 🚀 v14.0.1

1. Optimized the handling of connection failures and disconnects in collaboration mode;
2. Fixed an issue where the file related text was missing or incorrect, by @nishizbdjy in [(#458)](https://github.com/umodoc/editor/pull/458);
3. Fixed an issue where the block menu icons were not displayed correctly;
4. Improved the error message for FlowChart rendering.

## 🎉 v14.0.0

This release introduces the new **Document Form** capability. Umo Editor Next's document forms are not traditional standalone page forms. Instead, variable capabilities are embedded directly into the document body, allowing business systems to build a full workflow around **template design -> programmatic generation or manual filling -> data collection -> result export / viewing / archiving**.

It is also well suited for batch document generation. Business systems can pass different variable values into the same template to quickly generate multiple formal documents with consistent layout but different content, and then combine that with Umo Editor Next's export pipeline to produce Word, PDF, image, and other deliverables in batches.
...
1. Added documentation for the `collaboration.showLoading` option, which controls whether the first-sync loading overlay is shown during collaboration;
2. Added documentation and examples for `onCollaborationEvent`, which provides a unified way to listen to provider events;
3. Added documentation for the `localReady` and `connectionFailed` status fields returned by `getCollaboration()`.
...
This release syncs the latest changes from Umo Editor v11.0.0, with major improvements to tables, import/export experience, and large-document performance, along with fixes for collaboration, comments, and menu behavior. For details, see: [Umo Editor Changelog](../editor/changelog).

### 📊 Tables

1. Fixed alignment issues between the left row handles, top column handles, and row height/column width;
2. Row height and column width drag handles are now hidden in presentation mode;
3. Fixed issues where table height auto-sizing behaved incorrectly or could be unintentionally fixed after dragging row height;
4. Fixed an issue where the row height drag handle could stop working;
5. Optimized the visibility of row height and column width drag handles in read-only and editable states;
6. Wide tables that exceed the page width can now be viewed using a scrollbar;
7. Improved performance and interaction for tables in large documents and multi-table scenarios;
8. Focus is now automatically placed in the first cell when a table is created;
9. Fixed an issue where the number of column handles was not updated after adding table columns;
10. Significantly optimized table rendering performance to improve large-document loading speed.

### 📥 Import/Export

1. Optimized Word import results across different document structures;
2. Word export now supports exporting image descriptions;
3. Improved handling for incorrect line height when importing Office documents;
4. Added a prompt for large Office paste content;
5. Fixed incorrect block wrapping of inline nodes in annotations;
6. Fixed errors in Word export caused by color conversion issues;
7. Fixed issues that could occur when pasting Excel tables;
8. Optimized image export by reading rendered images directly from the DOM instead of loading them remotely, improving both speed and stability;
9. Improved Markdown table import with support for cell alignment.

### ⚡ Performance

1. Significantly optimized rendering performance for large documents, improving loading speed.

### 🧩 Other Fixes and Adjustments

1. Fixed an issue where collaboration parameters passed through the URL could fail;
2. Offline storage for collaboration is now disabled by default and can be enabled through [`collaboration.provider.enableIndexedDB`](./collaboration#);
3. Fixed an issue where menu items might occasionally fail to update dynamically;
4. Fixed an issue where saving could fail after closing comments;
5. Fixed an issue where the sticky tool menu could extend beyond the editor container;
6. Other minor refinements and adjustments.
...
1. Synced changes from Umo Editor v10.2.0. For details, see: [Umo Editor Changelog](../editor/changelog).

## 🎉 v11.0.0

1. Added document revision support. For details, see: [Document Revision](./revision);
...
1. Synced changes from Umo Editor v10.0.7 and v10.1.0. For details, see: [Umo Editor Changelog](../editor/changelog);
...
1. 🆕 Added the "Content Locking" feature. See: [Content Locking](./locked);
...
Note: if you upgrade to v10, the CDN assets referenced by `cdnUrl` (provided by [`@umoteam/editor-external`](https://www.npmjs.com/package/@umoteam/editor-external)) must also be upgraded to v10.

</Callout>

### Major upgrades

1. Synced code changes from Umo Editor v10. For details, see: [Umo Editor v10.0.0](../editor/changelog);
...
1. 🆕 Completely refactored AI features. Related APIs have major breaking changes and are incompatible with v9. Existing users should upgrade promptly. See: [AI Extension](./ai).
2. 🆕 Systematically enhanced Markdown support to better embrace AI. You can now directly import and export Markdown files. See: [Enhanced Markdown](./markdown).
3. 🆕 Added document version history management. See: [Version History](./versions).
4. 🆕 Added document annotation feature. See: [Document Annotation](./annotation).
5. 🆕 Added the ability to import web pages, allowing users to directly import content from any webpage 😂😂😂.
6. 🆕 Added real-time saving of page info (such as page size and background) in collaboration mode, and synced it to other collaborators in real time.
7. 🆕 Added Asset Library. You can pick images / videos / audio / documents from the asset library and insert them into the editor. See: [Asset Library](./file-picker).
8. Added the `floating_action` slot, see [Configuration & Methods - Slots](./ai/options#slots);
...
<Callout type="warning" emoji="❗">
  To maintain code synchronization with Umo Editor, Umo Editor Next adopts the same major version
  number as Umo Editor, starting directly from v7.0.0.
</Callout>
