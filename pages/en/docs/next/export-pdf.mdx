...
- `pdfOptions`：`Object`，Export PDF configuration items, optional, containing the following configuration items:
  - `pdfOptions.pageRanges`：`String`，Paper ranges to print, e.g. 1-5, 8, 11-13.
  - `pdfOptions.omitBackground`：`Boolean`，Hides default white background and allows generating pdfs with transparency.
  - `pdfOptions.scale`：`Number`，Scales the rendering of the web page. Amount must be between 0.1 and 2.
