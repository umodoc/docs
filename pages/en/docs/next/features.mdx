...
- Insert variables directly into the document body, including text, numbers, dates, radio buttons, checkboxes, selects, rich text, signatures, stamps, QR codes, barcodes, and images, making it easy to build standardized document templates;
- Support multiple working modes such as template design, manual filling, result viewing, and programmatic population, which works well for approval forms, registration forms, electronic medical records, notices, and similar scenarios;
- Collect variable values and final document results in a structured way, making it easier to connect saving, approval, archiving, and analytics workflows;
- Pass values through `form.values` or instance methods at runtime, which is especially useful for batch generation of formal deliverables such as PDF and Word files.
...
- Track insertions and deletions (text changes + node-level changes) for review and audit
- Review panel: see the change list, jump to the location, previous/next navigation
- Multiple view modes (`final-markup` / `final` / `original-markup` / `original`)
- Filter changes by author, which is useful for team reviews
- Accept/reject a single change or all changes to finalize

For details, see [Track Changes](./revision).
...
For details, see [Asset Library](./file-picker).
