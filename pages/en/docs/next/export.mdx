...
**Parameters:** `{ content, download, filename, docxOptions, exportComments, exportRevisions }`
...
- `docxOptions`: Options for document generation (optional), including:
...
> - Current Word export supports revisions for text content only. If the document contains more complex node-level revisions, we recommend finalizing it in the editor before export.
