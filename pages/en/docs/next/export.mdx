...
- `docxOption`: Options for document generation (optional), including:
  - `docxOption.getImageBase64`: `Async Function`, custom image loader, returns a valid base64 string. Parameters:
    ...
    - `nodeType: string` Type of image: `image` or `echarts`
    - `node: [Tiptap document JSON format]` Tiptap node
  - `docxOption.page.backColor`: `String`, background color for the exported document   `#FFFFFF`
  - `docxOption.page.orientation`：`String`,export paper orientation ：`landscape` or `portrait`
  - `docxOption.page.size.height`：`Number`,paper height in paper size, unit`cm`
  - `docxOption.page.size.width`：`Number`,paper width in paper size, unit`cm`
