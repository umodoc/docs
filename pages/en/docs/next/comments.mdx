...
![Umo Editor Document Comments](https://s2.umodoc.com/demos/comments-cn.png)
...
- Reviews and acceptance: propose changes on a paragraph and track whether it’s addressed
- Collaborative writing: discuss without inserting explanatory text into the body
- Contract/policy review: raise questions and risks clause by clause
- Document QA: flag structure issues in TOC, tables, images, footnotes, and more
- Teaching and coaching: leave point-by-point feedback with full traceability
- Requirements review log: keep decisions in threads for later audit
- Compliance checks: review sensitive wording, disclaimers, and legal statements
- Support/knowledge base maintenance: tag outdated content and assign an owner
- Meeting minutes confirmation: @mention owners to confirm key actions and responsibilities
- AI draft review: mark issues in AI-generated paragraphs and request revisions

## Works Well With

### Collaboration

- Keep discussion and editing decoupled while multiple people work in parallel: [Collaborative Editing](./collaboration)
- Threads sync across collaborators to avoid “only visible on one client” silos

### Track Changes

- Use comments for discussion and decisions, and Track Changes for the actual edits and finalization: [Track Changes](./revision)
- Converge by resolving comment threads first, then accept/reject changes to finalize

### Document Export

- When exporting to Word, you can optionally include comments, which is useful for review drafts with feedback: [Document Export](./export)
- Exported Word files preserve the comment and reply relationships within a thread for continued review

### Document Forms

- Use comment threads to discuss filling instructions, exceptions, approval notes, and other form-related issues: [Document Forms](./form)
- When saving a document, comments and form data can also be collected together through `onSave`, which is useful for approval and review workflows around forms

### Version History

- Create versions at key milestones and keep comments as part of the review record: [Version History](./versions)
- Use comment context to understand “why it changed” when comparing versions

### AI

- Generate a draft with AI, review it in comment threads, then apply the final edits via AI or manually: [AI](./ai)
...
If document forms are enabled at the same time, `onSave` will also append a fifth parameter, `form`, which is used to collect the current structured form data.

See the `onSave` method in [Document Save Options](../editor/options/methods#onsave).

Below is a simple example of an `onSave` method:

```js
const onSave = async (content, page, document, comments, form) => {
  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      console.log('onSave', { content, page, document, comments, form })
