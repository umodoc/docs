...
**Parameters**：`{ exportHtmlParams, exportType, imageOptions, filename, download }`
...
- `imageOptions`：`Object`，Export image configuration items, optional, containing the following configuration items:
  ...
  - `imageOptions.omitBackground`：`Boolean`，Whether to hide the default white background, setting to `true` can generate transparent background images, only effective for `png` format.
...
**Return Value**：`Blob | undefined`，When `download` is `true`, returns the exported image file binary data, when `download` is `true`, returns `undefined`.
