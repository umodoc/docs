...
<Callout type="error" emoji="❗">
  **Note:** The original `assistant` configuration has been changed to
  `ai.assistant` in version `v6.0.0`. Additionally, the `onAssistant` method has
  been renamed to `ai.assistant.onMessage`. Please update accordingly.
</Callout>
