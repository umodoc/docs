...
<Callout type="warning" emoji="⚠️">
  **Note**: After updating the `@umoteam/editor` version, please copy the latest
  `@umoteam/editor-external` files to the `public` directory. Ensure the version
  matches `@umoteam/editor-external`.
</Callout>

<Callout type="info">
  You can also place the files under `node_modules/@umoteam/editor-external/` in
  a suitable location and configure `cdnUrl` correctly. Ensure it does not end
  with a `/`.
</Callout>
