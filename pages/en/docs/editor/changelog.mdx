...
<Callout type="info" emoji="❗">
  This release is a **major milestone upgrade**. Without breaking existing configurations or usage patterns, the core of Umo Editor has been seamlessly migrated to **Tiptap v3**, along with a systematic refactoring of the overall codebase. All extensions have been upgraded to the latest v3 versions, references to all Pro extensions have been removed, and the entire codebase has undergone a systematic refactor.
  
  This upgrade significantly **reduces bundle size**, **improves loading and editing performance for large documents**, and lays a much more solid foundation for future feature expansion and long-term maintenance. **We strongly recommend all users upgrade to this version.**
</Callout>
...
2. Significantly reduced production bundle size, with an overall reduction of approximately **3–4×**;
3. Greatly improved large-document loading and editing performance—under the same benchmarks, supported document size has increased by **more than 3× (plain text)**;
4. Introduced a large number of new features—see the **“New”** section below for details;
5. Fundamentally resolved several long-standing minor issues.
...
3. Reworked the toolbar, menu system, and multiple core interaction logics to provide a cleaner and more extensible architecture.
...
1. Added a new **“View”** menu to the toolbar, including: outline view, fullscreen mode, presentation mode, view settings, zoom level, appearance settings, theme settings, and language settings. 👉 See the [Umo Editor Online Demo](https://www.umodoc.com/demo);
2. Introduced a brand-new **Formula** tool;
3. Added a new **Details** node;
4. Added **letter spacing** adjustment;
5. Added **text wrapping mode** settings;
6. Added **footnotes** support;
7. Enhanced **Mermaid** editing capabilities;
8. Added text alignment support for **Text Box** nodes;
9. The toolbar now supports horizontal scrolling via mouse wheel;
10. Improved compatibility when pasting **Office documents**;
11. Added document structure configuration options—see 👉 [document.structure](options/document#documentstructure);
12. Added node ID configuration for all node types. When enabled, document nodes can be located by ID—see 👉 [document.enableNodeId](options/document#documentenablenodeid);
13. Introduced a new skin system, along with events and methods for switching skins—see 👉 [setSkin method](./methods#setskin);
14. Added new `paste`, `drop`, and `delete` events—see 👉 [Events list](./events);
15. Added margin support to the `setPage` method—see 👉 [setPage method](./methods#setpage);
16. Introduced bundle analysis tools into the development build process to continuously optimize size and performance.

---

### 🐛 Fixes

1. Fixed an issue where the starting index of ordered lists was not applied correctly;
2. Updated the Indent extension to use `style` instead of `class` for indentation @tellbom [(#426)](https://github.com/umodoc/editor/issues/426);
3. Fixed several issues related to text boxes;
4. Fixed abnormal behavior during node dragging;
5. Fixed an issue where formulas might not render correctly after insertion;
6. Improved the display style of **Option** element nodes;
7. Fixed an issue where **Option** elements would occupy an entire line when printing, which did not match expectations @cheecheebaishu [(#407)](https://github.com/umodoc/editor/issues/407);
8. Fixed an issue where the bubble menu was displayed incorrectly when text within a table was selected;
9. Fixed an issue where image height could be dragged infinitely.

---

### ⚡ Improvements

1. Adjusted the overall menu order in the toolbar;
2. Improved bubble menu options for links, images, and other nodes;
3. Custom editable text nodes no longer display unnecessary line breaks by default;
4. Improved the visual presentation of the document outline;
5. Improved overall document rendering in preview mode;
6. Optimized editor content change listeners to reduce unnecessary page height recalculations;
7. Switched Mermaid, Plyr, and other third-party resources to CDN loading, further reducing bundle size [#401](https://github.com/umodoc/editor/issues/401) [#415](https://github.com/umodoc/editor/issues/415);  
8. Optimized the block menu handle styles;
9. Numerous minor tweaks and optimizations.

---

### 🧹 Removed

1. Removed the `contentError` event;
2. Removed **Electronic Signature** and **Image Background Removal** features due to third-party resource size considerations [#401](https://github.com/umodoc/editor/issues/401) [#415](https://github.com/umodoc/editor/issues/415);
3. Removed the document preview feature to further optimize overall bundle size [#401](https://github.com/umodoc/editor/issues/401) [#415](https://github.com/umodoc/editor/issues/415).
4. Removed some configuration items from `disableExtensions`. See: [disableExtensions configuration](./options/extensions).
...
8. Removed `toolbar.disableMenuItems` and `toolbar.importWord` options, replaced with new [`disableExtensions`](./options/extensions#disableextensions) and [`importWord`](./options/import-word#importword) configurations;
