...
1. Fixed an issue where the file related text was missing or incorrect, by @nishizbdjy in [(#458)](https://github.com/umodoc/editor/pull/458);
2. Fixed an issue where the block menu icons were not displayed correctly.

## 🚀 v11.1.0  <Badge>July 31, 2026</Badge>

1. Delayed the autofocus timing of the Find and Replace component, @hszl000 [(#457)](https://github.com/umodoc/editor/issues/457);
2. Fixed an issue where the Find and Replace panel could appear in an unexpected position;
3. Fixed an issue where large images could exceed the page width;
4. Fixed an occasional issue where dragging an image could duplicate the same node;
5. Fixed an issue where floating images could still be dragged in read-only mode;
6. Improved the QR code / barcode tools; added local upload and cropping support to the signature tool; added a new stamp tool; and more;
7. Fixed `outside of fragment|out of range xxx` errors that could occur when setting content;
8. Improved the display behavior of the toolbar in scrollable scenarios;
9. Other minor refinements and optimizations.
...
## 🎉 v11.0.0 <Badge>July 19, 2026</Badge>
...
### 🛠 Core Stability and API Fixes

1. Fixed issues that could occur when page layout was set to the web layout only;
2. Fixed an error in [`setContent`](./methods#setcontent) where an invalid selection could lead to an out-of-range position;
3. Fixed an issue where `setOptions` could fail to update some configuration items dynamically;
4. Fixed the typo in the `getOpitons` method name; the correct name is [`getOptions`](./methods#getoptions);
...
### 🖼 Image, File, and Resource Handling
...
### 💬 Mentions, Footnotes, and Supporting Information

1. The mention (`@user`) feature now supports loading user lists from configuration or remote sources, adds the [`onMentionSearch`](./options/users#onmentionsearch) option, and adds a `bio` field to `users` for displaying user bios or other extra information;
...
### 📋 Paste and Content Parsing Compatibility
...
### 📐 Layout, Columns, and Editing Experience
...
5. Fixed an issue where line height could be lost when inserting a heading block into an empty line, contributed by @ohmua in [(#455)](https://github.com/umodoc/editor/pull/455);
...
### 📑 Lists, Outline, and Other Improvements
...
4. Fixed an issue where pasted content could be duplicated, @jiangshan1xiao [(#447)](https://github.com/umodoc/editor/issues/447); @lylezhan [(#440)](https://github.com/umodoc/editor/issues/440);
...
12. Fixed unexpected line breaks in list items, @hzy-GitH [(#448)](https://github.com/umodoc/editor/issues/448);
13. The image extension now supports reading width/height from `style`, @wuyuejiang482 [(#445)](https://github.com/umodoc/editor/issues/445);
...
3. Added font download and auto-enable support. See [Dict Configuration](./options/dicts#fonts);
...
1. Fixed the issue where nodes could not be dragged due to a bug in the official Tiptap extension, @BOKE2014 [(#433)](https://github.com/umodoc/editor/issues/433).

## 🎉 v10.0.0 <Badge>February 28, 2026</Badge>
...
2. Removed the little-used Word import feature. Note: related configuration items have also been removed—please delete them from your configuration, otherwise errors may occur.
3. Removed the `contentError` event;
4. Removed **Electronic Signature** and **Image Background Removal** features due to third-party resource size considerations [#401](https://github.com/umodoc/editor/issues/401) [#415](https://github.com/umodoc/editor/issues/415);
5. Removed the document preview feature to further optimize overall bundle size [#401](https://github.com/umodoc/editor/issues/401) [#415](https://github.com/umodoc/editor/issues/415);
6. Removed some configuration items from `disableExtensions`. See: [disableExtensions configuration](./options/extensions).
...
## 🎉 v9.0.0 <Badge>December 5, 2025</Badge>
...
1. [Umo Editor Next 7.0.0](../next/changelog) official release is now available 🎉🎉🎉. For details, see: [Introduction to Umo Editor Next](../next).
...
19. For Nuxt users, [Umo Editor Nuxt](https://www.npmjs.com/package/@umoteam/umo-editor-nuxt) has been released.
...
1. Added i18n support, currently supporting Simplified Chinese and English, see [`locales`](./locales);
...
4. Some API changes have occurred, the main changes are as follows:
   ...
   - Added `locale` and `translations` configurations, see [Custom Localization Configuration](./locales);
