...
- `POST /mock/sse/normal`: AG-UI SSE example interface, returns `text/event-stream`. The `data` field of each SSE is AG-UI event JSON. This interface is for demonstration purposes to facilitate joint debugging of frontend AG-UI parsing and rendering, and will not call the upstream model.
