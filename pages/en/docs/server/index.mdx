---
target: Umo Editor Server
title: Umo Editor Server Introduction
keywords: Umo Editor Next,Umo Editor Server,Umo Editor Backend,Umo Editor Server-side
description: Umo Editor Server is a companion server-side software developed to enhance some features of Umo Editor Next, such as multi-user online collaboration, document comments, and document import/export.
---

# Introduction

Umo Editor Server is a companion server-side software developed to enhance some features of Umo Editor Next, such as multi-user online collaborative editing, document comments, document import/export, etc. If you have already purchased the commercial license for Umo Editor Next, you can use Umo Editor Server for free under the [Umo Team Commercial Software License Agreement](./license).

Umo Editor Server is a Node.js server-side software based on [Express](https://expressjs.com/?hl=zh-CN). Its main function and purpose is to enhance the capabilities of Umo Editor Next through HTTP or WebSocket interfaces, supporting private deployment.

Umo Editor Server is not distributed as an NPM package. After obtaining a commercial license, you can directly pull the source code from the private repository and modify and deploy it as needed.

## Start the Project

### 1. Clone the Official Repository

```bash
git clone <Umo Editor Server official private repository URL>
```

### 2. Install Dependencies

Please refer to the "Installing Dependencies" section in the project's `README.md` document, **make sure to install the required dependencies according to the instructions**.

### 3. Configure Environment Variables

Umo Editor Server does not provide unified configuration items. Some sensitive configurations are set through environment variables. Continuing the philosophy of Umo Editor's out-of-the-box experience, you only need to add a `.env` file in the project root directory and add environment variable information to quickly start. Here is an example of environment variables:

```bash filename=".env"
NODE_ENV = development

## Service related
PORT = 1235
SERVER_NAME = Umo Editor Server

## Log related
LOG_DIR = ./logs

## Database related
# All database connections use COLLAB_DB_URL
# SQLite (default)
COLLAB_DB_URL = sqlite:./db/umo-editor-server.sqlite
#
# MySQL
# COLLAB_DB_URL = mysql://root:password@127.0.0.1:3306/umo_editor_server
#
# PostgreSQL
# COLLAB_DB_URL = postgres://postgres:password@127.0.0.1:5432/umo_editor_server
#
# MongoDB
# COLLAB_DB_URL = mongodb://user:password@127.0.0.1:27017/umo_editor_server
#
# S3 / MinIO
# COLLAB_DB_URL = s3://your-bucket/umo_editor_server?region=ap-southeast-1
# COLLAB_DB_URL = s3://your-bucket/umo_editor_server?region=us-east-1&endpoint=http://127.0.0.1:9000&forcePathStyle=1

## Collaboration related
DOC_SAVE_INTERVAL = 5000 # Unit: milliseconds

## AI related
DEEPSEEK_API_KEY = your-api-key
DEEPSEEK_API_URL = https://api.deepseek.com/v1
OPENAI_API_KEY = your-api-key
OPENAI_API_URL = https://api.openai.com/v1
```

You can also copy this information directly from `.env.example`.

### 4. Start the Project

```sh npm2yarn
npm run dev
```

### 5. All Set

After the above steps, you have successfully started Umo Editor Server. Visit http://127.0.0.1:1235/openapi to view the open api documentation.

## Development Notes

- For scenarios such as multi-user collaborative editing, document comments, and document version history, Umo Editor Server provides detailed configuration and hook methods. By default, SQLite is used to store data (you can switch to MySQL/PostgreSQL/MongoDB/S3/MinIO via `COLLAB_DB_URL`). You can integrate or communicate with existing business systems through `database`, `redis`, `sqlite`, `webhook`, etc. For details, see the code comments.
- You can deploy Umo Editor Server as a separate service and integrate or communicate with other business systems through gateways, databases, HTTP requests, etc. Umo Editor Server currently does not include authentication implementation (but provides implementation ideas and sample code, see [Authentication Instructions](./server/auth)). If needed, please implement it yourself.
- If you need secondary development, the source code provides a large number of detailed comments that should be very helpful to you.

## Deployment Notes

- In an environment that supports `nodejs`, you can directly run the project using `npm run production`. This command provides the ability to use the `[pm2](https://pm2.io/)` daemon and record logs.
- You can also deploy the project directly to a `docker` container. You can modify the `Dockerfile` as needed. For details, see [Docker Deployment](./server/docker).

## Products from Umo Team

- [Umo Editor](https://dev.umodoc.com/en/docs/editor): A localized open-source document editor based on Vue3 and Tiptap.
- [Umo Viewer](https://dev.umodoc.com/en/docs/viewer): An open-source, lightweight document viewer for Umo Editor.
- [Umo Editor Mobile](https://mobile.umodoc.com): A mobile document editor for enterprise business scenarios, designed for standalone integration as well as collaboration with Umo Editor and Umo Editor Next.
- [Umo Editor Next](https://dev.umodoc.com/en/docs/next): The enterprise edition of Umo Editor, built for teams that need more than a rich-text editor. It extends the core editing experience with a more complete foundation for real business workflows, making it a better fit for products that need to bring document capabilities into complex enterprise scenarios.
- [Umo Editor Server](https://dev.umodoc.com/en/docs/server): A companion server-side software developed to extend Umo Editor capabilities such as multi-user collaborative editing, document annotations, and document import/export.
- [Umo Office Viewer](https://dev.umodoc.com/en/docs/office-viewer): An Office document viewer that supports previewing over 40 mainstream office document in web pages.
- [Umo Office Convert](https://dev.umodoc.com/en/docs/office-convert): Converts over 40 office formats into web-viewable formats, ready to pair with Umo Office Viewer for seamless online document preview.
