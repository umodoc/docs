...
Umo Editor Next's changelog can be found at: https://editor.umodoc.com/cn/docs/next/changelog

1. Add support for Markdown image input and pasting;
2. Fix the issue where the bubble menu may not display completely;
3. Fix the issue where the bubble menu does not display after exiting the format painter;
...
5. Fix the issue where when file extensions are not fully matched, alignment fails @liushiji123 [(#254)](https://github.com/umodoc/editor/issues/254)；
6. Add bubble menu for link nodes;
7. Lock version dependencies to prevent errors caused by inconsistent `prosemirror-mode` versions @BOKE2014 [(#257)](https://github.com/umodoc/editor/issues/257)。
