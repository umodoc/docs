...
<Callout type="error">
Starting from version v2.0.0, the `label` in `dicts` and `document.placeholder` support both `String` and `Object` types. The `Object` type is mainly used for configuring multilingual information in the format `{ en_US: 'English', zh_CN: '中文说明' }`.
</Callout>
...
- `fullscreenZIndex`: Number, the z-index of the container in fullscreen mode, used to prevent being covered by other elements, such as when used in a popup. <Badge theme="success">Added in v6.0.0</Badge>
...
- `assistant`: Object, AI document assistant configuration, see [AI Document Assistant](./assistant). <Badge theme="success">Added in v3.0.0</Badge>
...
- `translations`: Object, see [Custom Localization Language Configuration](./locales). <Badge theme="success">Added in v2.0.0</Badge>
...
- `onAssistant(payload, content)`: Async Function, AI document assistant return data method, see [Method Configuration](./methods#onassistant). <Badge theme="success">Added in v3.0.0</Badge>
