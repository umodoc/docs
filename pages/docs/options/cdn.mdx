...
```js
{
  cdnUrl: '//unpkg.com/@umoteam/editor-external@latest',
}
```
...
```js
{
  ...
  cdnUrl: '//unpkg.com/@umoteam/editor-external@latest',
  // 或使用 jsdelivr.net
  // cdnUrl: '//cdn.jsdelivr.net/npm/@umoteam/editor-external@latest',
}
```

## 使用本地资源

对想进一步提高加载速度，或者内网环境等场景，您可以将 `node_modules/@umoteam/editor-external/` 文件夹下的文件拷贝到 `public/` 下，并设置 `cdnUrl` 为 `location.origin` 即可。
...
**注意**：每次更新 `@umoteam/editor` 版本后，请及时复制最新的 `@umoteam/editor-external` 到 `public` 目录下。注意和 `@umoteam/editor-external` 版本号保持同步。
</Callout>
...
**默认值**：`//unpkg.com/@umoteam/editor-external@latest`
