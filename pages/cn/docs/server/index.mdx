...
通过上述操作，您已经成功启动了 Umo Editor Server，访问 http://127.0.0.1:1235/openapi 即可查看 Open API 文档。

## 开发说明

- 对于多人协作编辑、文档批注(评论)、文档历史版本管理等场景，Umo Editor Server 提供了详尽的配置和钩子方法，默认采用 SQLite 存储数据（通过 `COLLAB_DB_URL` 可切换为 MySQL/PostgreSQL/MongoDB/S3/MinIO 等），您也可以通过 `database`、`redis`、`sqlite`、`webhook` 等扩展和现有业务系统对接或通信，详细说明见代码注释；
