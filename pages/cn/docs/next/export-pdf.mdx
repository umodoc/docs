...
注意：如果未配置 `export.serverURL`，则在用户点击工具栏中的导出 PDF 按钮时，会降级使用前端导出。
...
- `pdfOptions`：`Object`，导出 PDF 的配置项，可选，包含以下配置项：
  - `pdfOptions.pageRanges`：`String`，要打印的纸张范围，例如 1-5, 8, 11-13。
  - `pdfOptions.omitBackground`：`Boolean`，是否隐藏默认的白色背景，设置为 `true` 可以生成具有透明度的 PDF。
  - `pdfOptions.scale`：`Number`，缩放比例，必须介于 0.1 和 2 之间。
