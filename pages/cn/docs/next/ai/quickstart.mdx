...
2. 一个可访问的后端 endpoint（推荐 SSE）：`ai.models[].endpoint`，前端会向该 endpoint 发起请求，你可以先行连接到 Umo Editor Server 提供的默认 SSE 示例服务 `/mock/sse/normal` 或 `/mock/sse/agui` 进行测试，见：[接口列表](../../server/api)。
