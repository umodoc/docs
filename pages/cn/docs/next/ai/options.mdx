...
export const defaultAIOptions = {
  ...
  skills: ['write', 'image', 'code', 'katex', 'mermaid', 'search'],
  maxlength: 500,
  retryInterval: 5000,
  maxRetries: 3,
  timeout: 10000,
...
**说明**：允许的技能模式列表，决定发送框技能下拉里可切换的模式，可选值为 `['write','image','code','katex','mermaid','search']`，其中每个项目的说明如下：

- `write`：文档助手，编辑或生成文本 / Markdown。
- `image`：图像生成，仅根据用户描述生成图像内容。
- `code`：代码助手，专注于代码生成、补全、重构或解释。
- `katex`：公式助手，专注于数学公式、逻辑推导与计算。
- `mermaid`：图表助手，专注于流程图、UML、架构图等结构化图表。
- `search`：网页解读，对网页或页面内容进行理解、总结或解释。

**类型**：`Array`

**默认值**：`['write','image','code','katex','mermaid','search']`
...
**说明**：发送请求前的统一改写入口。可以在这里注入鉴权 headers、改写 body、或把前端 `params` 映射为后端协议。
...
**说明**：自定义协议（`model.protocol !== 'agui'`）下，用于把后端 SSE chunk 映射为聊天内容块（markdown / thinking 等）。

**类型**：`(context, chunk) => { type: string, data: any } | null`

**默认值**：无（未配置时会使用内置兜底：读取 `chunk.data.msg` 并作为 markdown 渲染）
...
**说明**：获取 AI 引擎状态对象（Vue ref）。AI 文档助手与 AI 聊天助手共用同一个聊天引擎状态对象，可以用它读取当前模型 / 消息 / 状态等信息和聊天引擎实例等，或在业务侧做更深度的 UI 联动与调试。
...
- `options`: `Object`，当前生效的 `options.ai` 配置对象。
- `model`: `Object`，当前选中的模型（来自 `options.ai.models[]`）。
...
- `engine`: `Object`，Umo Editor Next 内部的聊天引擎对象。您可以利用 `engine` 进行更复杂的场景定制，如发送消息、调试与埋点等。
