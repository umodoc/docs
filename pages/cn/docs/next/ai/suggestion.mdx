...
AI 建议是一种基于上下文的智能提示能力，用于在用户输入过程中实时预测并生成后续内容。它能够根据当前文本及光标位置，提供连续写作、智能提示或文本补全建议，帮助用户减少重复输入，提高编辑效率与写作流畅度。

AI 建议功能适用于 AI 写作、智能提示、代码补全、文本增强等场景。目前，AI 建议仅适用于文本类节点。
...
**说明**：AI 建议的核心回调函数，用于根据当前文本上下文生成补全建议。

**类型**：`AsyncFunction`,`Promise`

**参数**：

- `params.text`：当前节点完整的整文本内容，包含当前光标位置，用于精确告知 AI 建议的当前光标位置和上下文内容，适合LLM 补全、上下文分析、续写判断等。当前光标位置用 `ai.cursorPlaceholder` 中配置的占位符表示。
- `params.node`：当前节点的 Markdown 内容。
- `params.document`：整个文档的 Markdown 内容。
...
**说明**：用户输入时触发 AI 建议的防抖时间，用于控制补全请求的触发频率，单位为毫秒。
...
**说明**：在编辑器已就绪且启用 AI 建议（`ai.suggestion.enabled` 为 `true`）的情况下，开启 AI 建议功能。
...
**说明**：在编辑器已就绪且启用 AI 建议（`ai.suggestion.enabled` 为 `true`）的情况下，关闭 AI 建议功能。
...
**说明**：在编辑器已就绪且启用 AI 建议（`ai.suggestion.enabled` 为 `true`）的情况下，切换 AI 建议功能的开启和关闭状态。
