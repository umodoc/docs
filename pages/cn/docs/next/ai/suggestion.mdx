...
- `params.text`：当前节点完整的整文本内容，包含当前光标位置，用于精确告知 AI 智能建议的当前光标位置和上下文内容，适合LLM 补全、上下文分析、续写判断等。当前光标位置用 `ai.cursorMarkder` 中配置的占位符表示。
- `params.node`：当前节点的 Markdown 内容。
- `params.document`：整个文档的 Markdown 内容，包含当前光标位置，当前光标位置用 `ai.cursorMarkder` 中配置的占位符表示。
