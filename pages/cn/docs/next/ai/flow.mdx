...
```js
{
  ...
  skill,            // 用户所选的技能名称：'write' | 'image' | 'code' | 'katex' | 'mermaid' | 'search'
...
- `from === 'chat'`
- `options.ai.chat.files.enabled === true`
- 用户已上传且上传完成（文件状态不是 progress）

`attachments` 来自 `options.onFileUpload` 返回信息。
...
最低要求：每个 SSE 事件输出一行 JSON（不要直接输出带换行的 Markdown）。
...
如果准备做“工具调用 / 多步骤任务 / 进度状态”，建议优先 AG-UI。
