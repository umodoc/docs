...
- 推荐做法：在后端实现一个 **适配层/代理**：前端的 `default` 或 `agui` 协议访问后端，后端再去调用 OpenAI 兼容接口，并把返回转换为 SSE 事件回推给前端，可以参考或直接调用 Umo Editor Server 中的 [AI 服务代理接口](../../server/api#ai-服务代理) 接口。
...
- 不会走 `ai.callbacks.onMessage` 的自定义解析
