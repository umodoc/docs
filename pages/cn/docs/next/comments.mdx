...
- 导出 Word 时可按需包含批注，适合输出带审阅意见的评审稿：[文档导出](./export)
- 导出的 Word 会保留线程中的评论与回复关系，便于后续继续审阅

### 文档表单（form）

- 对填写说明、异常项、审批意见等内容，可通过评论线程沉淀讨论过程：[文档表单](./form)
- 保存文档时，评论与表单数据也可以在 `onSave` 中一起回收，适合表单审批与审阅场景
...
如果同时开启了文档表单能力，`onSave` 还会继续追加第 5 个参数 `form`，用于回收当前表单结构化数据。
