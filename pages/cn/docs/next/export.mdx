...
此功能允许将 Umo Editor 中的内容导出为 Word、PDF、HTML、图片等文件并保留原格式。我们正在积极完善该功能，尤其是针对 Word 导出功能的优化。
...
**说明**：在导出部分文件时，需要正确的配置 Umo Editor Server 的 URL，Umo Editor Server 提供了相关导出的 API，您需要确保 Umo Editor Server 已经启动，当然您也可以在服务端自行实现导出功能，这里填写正确的 URL 即可。

**类型**：`String`

**默认值**：`''`

**示例**：`http://127.0.0.1:1235`
...
- `docxOption`：控制文档生成过程的配置信息，可选，包含以下配置项：
  ...
  - `docxOption.page.backColor`：`String`，导出文档背景色 比如  `#FFFFFF`
