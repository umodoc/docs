...
- `onAssistant(payload, content)`：Async Function，AI 文档助手返回数据方法，详见[AI 助手配置](./methods#onassistant)。 <Badge theme="success">v3.0.0 新增</Badge>
- `onCustomEChartSettings(data, config)`：Function，自定义 ECharts 图表配置方法，详见[方法配置](./methods#oncustomcehartsettings)。 <Badge theme="success">v6.0.0 新增</Badge>
- `onCustomImportWordMethod(file)`：Async Function，自定义 Word 文件导入方法，详见[方法配置置](./methods#oncustomimportwordmethod)。 <Badge theme="success">v3.1.0 新增</Badge>
