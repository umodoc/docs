...
用于配置 Umo Editor 的方法，包括文档保存方法、文件上传方法、文件删除方法等。
...
在 Umo Editor Next 中，如果同时开启了批注或文档表单等能力，`onSave` 还会在后续参数中追加对应的结构化数据。例如：

- 第 4 个参数 `comments`：当前批注线程列表
- 第 5 个参数 `form`：当前表单结构化数据

**返回值**：保存成功则返回 `true`，否则返回 `false` 或者抛出错误。
...
- `id`：文件 id。
- `url`：文件 url。
- `type`：文件类型标记，用于区分文件、图片、音频、视频等资源类型。
