...
除此之外，如果要上传同一类型的文件，可以使用 _ 来简化配置，如允许上传所有图片，可配置为 `['image/_']`。
...
- `preview[].onPreview(file)`: 文件预览方法，如果配置了此方法，则在用户点击文件预览按钮时，会优先调用此方法而忽略预览链接，利用此方法可拦截内部预览窗口，通常用于自定预览组件的场景等。<Badge theme="success">v10.0.0 新增</Badge>
