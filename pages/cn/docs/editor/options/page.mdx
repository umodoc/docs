...
**说明**：可选的页面布局列表，可设为页面模式(page)和 Web 模式(web)，默认使用数组第 1 项作为初始布局，用户可自由选择。如果你想仅使用页面模式，将其设为`['page']`即可，仅使用 Web 模式设为 `['web']`。您可以通过[setLayout 方法](../methods#setlayout)或者[setPage 方法](../methods#setpage)来动态修改默认布局。<Badge theme="success">v8.0.0 新增</Badge>
...
- `fontFamily`: `String`，字体，合法的[CSS font-family](https://developer.mozilla.org/zh-CN/docs/Web/CSS/font-family)值。
