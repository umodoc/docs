...
**说明**：文档 ID，在 Umo Editor Next 版本的多人协作场景下，文档 ID 是必填的，用于唯一标识一个文档。
...
**说明**：配置文档结构，利用此选项，您可以强制文档的结构，例如要强制首项必须为文档标题，此项可以配置为：`heading block*`。这里是 Tiptap 的示例：https://tiptap.dev/docs/examples/advanced/forced-content-structure <Badge theme="success">v10.0.0 新增</Badge>
...
**说明**：是否为文档节点生成唯一标识，为了获得更好的性能，此项默认关闭。 <Badge theme="success">v10.0.0 新增</Badge>
...
**说明**：定义[ProseMirror](https://prosemirror.net/docs/ref/#model.ParseOptions)解析行为，当你需要解析一个 DOM 节点或者一个原始文本字符串到 ProseMirror 的文档模型时，你可以使用 `document.parseOptions` 来自定义解析过程。更多信息见：https://prosemirror.net/docs/ref/#model.ParseOptions 。 <Badge theme="success">v2.3.0 新增</Badge>
