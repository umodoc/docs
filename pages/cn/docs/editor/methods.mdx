...
**说明**：设置 Umo Editor 的布局信息，可设为页面视图或者 Web 视图，设置为某种布局时，需要[`page.layouts`](./options/page#pagelayouts)中包含该布局，否则设置不成功。
...
- 请优先使用对应 `document.*` 配置中的真实字段名，例如 `enableBubbleMenu`、`enableMarkdown`、`enableSpellcheck`。
...
**说明**：获取 Tiptap Editor 实例。 <Badge theme="success">v4.6.0 新增</Badge>
