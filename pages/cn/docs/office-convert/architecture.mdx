...
Umo Office Convert 是一个基于 Node.js 与 Express 构建的文档转换服务，将各类办公文档转换为 [Umo Office Viewer](../office-viewer) 支持查看的格式。系统同时集成 MySQL 用于持久化转换任务信息，并支持跨域访问与 Swagger 接口文档。
...
- 文档：Swagger（在非生产环境挂载 `/openapi`），见：[API 接口文档](./api)
- 日志：通过环境变量`LOG_DIR` 指定日志目录（PM2 可选）
