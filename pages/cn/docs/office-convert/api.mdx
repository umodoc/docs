...
本文档详细描述了 Umo Office Convert 服务提供的 API 接口。该服务主要用于将各类 Office 文档和 PDF 文档转换为 [Umo Office Viewer](https://dev.umodoc.com/cn/docs/office-viewer) 可查看的格式。

## 基础信息

- 基础URL: `http://localhost:1236`（默认端口，可在环境变量中配置）
- 在非生产环境下，可通过 `/openapi` 路径访问 Swagger UI 接口文档
...
根据转换任务 ID 获取文件封面（即文件第一页）。

- **URL**: `/convert/{taskId}/cover?size={size}`
- **方法**: `GET`
- **参数**:
  - `taskId`: 转换任务 ID（路径参数）
  - `size`: 封面大小（查询参数）
...
服务支持转换多种 Office 文档格式，所支持的格式见：[支持转换的文件格式](./#支持转换的文件格式)。
...
服务支持通过 Webhook 接收文件转换完成的通知。可以在环境变量中配置 `CONVERTED_WEBHOOK_URL`，详见：[环境变量-Webhook 配置](./env#webhook-配置)。
