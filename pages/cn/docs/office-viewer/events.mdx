...
<script setup>
  ...
  const onAsideChange = (aside: 'info' | 'thumbs') => {
    console.log('emit:aside-change', '开启侧边栏并切换面板:', { aside })
  }
  const onPageChange = (activePage: number) => {
    console.log('emit:page-change', '当前页码变化:', { activePage })
  }
  const onRotateChange = (rotate: number) => {
    console.log('emit:rotate-change', '旋转角度发生变化:', { rotate })
  }
  const onScaleChange = (scale: number) => {
    console.log('emit:scale-change', '缩放比例变化:', {scale})
  }
  const onFullscreenChange = (fullscreen: boolean) => {
    console.log('emit:fullscreen-change', '全屏状态发生变化:',  { fullscreen })
  }
  const onDownload = () => {
...
| aside-change | 开启侧边栏并切换面板后 | aside |
| page-change | 当前页码变化后，鼠标滚动或插件内部触发  | activePage |
| rotate-change | 页面旋转角度变化后  | rotate |
| scale-change | 页面缩放比例变化后  | scale |
| fullscreen-change | 插件全屏状态切换后   | fullscreen |
| download | 下载文档  | - |
| export | 导出文档  | - |
| print | 打印文档  | - |
